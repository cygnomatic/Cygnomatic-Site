...
3.  **资源管理与矩阵运算:**
    ...
    *   **具体实现要求:**
        1.  **构造函数:** 实现一个构造函数，接收行数 `rows` 和列数 `cols`，并动态分配所需的内存（例如，使用 `new double[rows * cols]`）。
        2.  **析构函数:** 实现析构函数，正确释放构造函数中分配的内存，防止内存泄漏。
        3.  **禁用拷贝:** 使用 `= delete` 语法**显式禁用**拷贝构造函数和拷贝赋值运算符。
        4.  **实现移动:** 正确实现移动构造函数和移动赋值运算符。在移动操作后，原对象应处于一个有效的、可被安全析构的状态（例如，其内部指针为空）。
        5.  **元素访问:** 重载 `operator()`，使得可以通过 `matrix(row, col)` 的方式方便地访问和修改矩阵中的元素。
        6.  **（选做）** 提供 `get_rows()` 和 `get_cols()` 成员函数返回矩阵的维度。
    *   **测试要求:** 提供一个 `test.cpp`
        *   展示`Matrix`的正确创建和销毁。
