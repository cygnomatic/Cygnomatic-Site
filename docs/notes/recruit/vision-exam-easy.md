...
1.  **场景:**
    给定一个由`0`和`1`组成的二维地图，`0`代表平地（可通行），`1`代表障碍物（不可通行）。机器人只能在平地区域移动（上下左右四个方向）。

2.  **实现:**
    
    *   **语言:**Python/C++。
    *   **算法:** 使用**广度优先搜索 (BFS)** 或**深度优先搜索 (DFS)** 算法。
    *   **任务:** 编写一个函数或程序，它接收一个**命令行参数**作为地图文件的路径，读取地图，并判断从**左上角 (0,0)** 到**右下角 (4,4)** 是否存在一条可通行的路径。
    *   **正式测试文件 `map.txt` 内容:**
        请在你的项目中创建 `map.txt` 文件，并使用以下内容作为其唯一的、正式的测试数据：
    ...
    *   **测试要求:** 你的 `test.sh` 脚本应能编译（如果需要）并像这样运行它：`./your_program_name map.txt`。
